...
def main():
    ...
    ff =  open("finalpositions.txt","a") #opening file to write index, stop time, position, velocity' at the time when the particle stops
    ...
    for runs in range(0,100): # runs start here
        ...
        for iterations in range(0,no_of_iterations):
            ...
            velocity = update_velocity() # update velocity
            position = update_pos() # update position
    ...
    #plt.show()
    fig.savefig('trajectory.png') #saving figure as trajectory.png

    fig.clear() #clearing figure for writing histogram

    bins = [0,2,4,6,8,10] # bins for the histogram
    plt.hist(stop_time, bins, histtype= 'bar',rwidth = 0.8 ) # plotting the histogram

    fig.savefig('histogram.png') #saving the histogram
